...
Particle::Particle(const ParticleParameter& pp, const glm::vec3& pos, const Rect& r) :
	rect(r), position(pos), lifetime(pp.lifeTime), velocity(pp.velocity),
	acceleration(pp.acceleration), scale(pp.scale), rotation(pp.rotation), color(pp.color)
{
}
...
* コンストラクタ.
*
* @param ep  エミッターの初期化パラメーター.
* @param pp  パーティクルの初期化パラメーター.
*/
ParticleEmitter::ParticleEmitter(const ParticleEmitterParameter& ep, const ParticleParameter& pp) :
	ep(ep), pp(pp), interval(1.0f / ep.emissionsPerSecond)
{
...
void ParticleEmitter::AddParticle()
{
	...
	// velocityだけ変更したパーティクルの初期化用パラメーターを作成.
...
bool ParticleSystem::Init(size_t maxParticleCount) 
{
	...
	vao.Bind();
	vao.VertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), offsetof(Vertex, center));
	...
	vao.VertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), offsetof(Vertex, offset));
	vao.Unbind();
	...
	program = Shader::Program::Create("Res/Shaders/Particle.vert", "Res/Shaders/Sprite.frag");
	if (!program) {
		std::cerr << "[エラー] パーティクル・システムの初期化に失敗.\n";
		return false;
	}
...
void ParticleSystem::Update(float deltaTime) 
{
	...
	for (auto& e : emitters) {
		...
		for (auto& particle : e->particles) {
			// 座標変換行列を作成.
			const glm::mat4 matR = glm::rotate(glm::mat4(1), particle.rotation, glm::vec3(0, 0, 1));
