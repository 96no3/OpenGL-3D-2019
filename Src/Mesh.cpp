...
namespace Mesh {
	...
	*/
	glm::quat GetQuat(const json11::Json& json) {
		...
		return glm::quat(static_cast<float>(a[3].number_value()),
	...
	*/
	glm::quat GetMat4(const json11::Json& json) {

		const std::vector<json11::Json>& a = json.array_items();
		if (a.size() < 16) {
			return glm::mat4(1);
		}
		return glm::mat4(a[0].number_value(), a[1].number_value(), a[2].number_value(), a[3].number_value(),
	...
	*/
	void GetBuffer(const json11::Json& accessor, const json11::Json& bufferViews, const std::vector<std::vector<char>>& binFiles,
				   const void** pp, size_t* pLength, int* pStride = nullptr) {
	...
	*/
	bool Buffer::Init(GLsizeiptr vboSize, GLsizeiptr iboSize) {
		...
		progStaticMesh = Shader::Program::Create("Res/Shaders/StaticMesh.vert", "Res/Shaders/StaticMesh.frag");
	...
	* @retval false			設定失敗.
	*/
	bool Buffer::SetAttribute(Primitive* prim, int index, const json11::Json& accessor, const json11::Json& bufferViews, 
							  const std::vector<std::vector<char>>& binFiles) {
	...
	*/
	bool Buffer::LoadMesh(const char* path) {
		...
		for (const json11::Json& buffer : json["buffers"].array_items()) {
			...
			const std::string binPath = std::string("Res/Models/") + uri.string_value();
			...
			for (const json11::Json& material : materials) {
				...
				if (index.is_number()) {
					...
					if (imageName.is_string()) {
						texturePath = std::string("Res/Models/") + imageName.string_value() + ".tga";
					}
				}
