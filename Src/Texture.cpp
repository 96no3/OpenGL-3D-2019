...
#define MAKE_FOURCC(a, b, c, d)\
	...
	*/
	struct DDSHeader 
	{
		...
		uint32_t flags;				///< どのパラメータが有効化を示すフラグ.
		...
		uint32_t mipMapCount;		///< 含まれているみっぷマップレベル数.
	...
	*/
	GLuint LoadDDS(const char* filename) 
	{
		...
		GLenum iformat = GL_RGBA8; // GPUがテクスチャを読み取る形式
		GLenum format = GL_RGBA;   // 実際のデータ形式
		uint32_t blockSize = 16;   // 圧縮フォーマットのデータ格納単位となるバイト数
		bool isCompressed = false; // 圧縮形式かどうか
	...
	* ファイルから2Dテクスチャを読み込む.
	*
	* @param path	     2Dテクスチャとして読み込むファイル名.（画像として読み込むファイルのパス）
