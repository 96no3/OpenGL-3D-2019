...
namespace Terrain {
	...
	* @param baseLevel	 高さを0とみなす高さ値（色データ0.0～1.0のどこを高さ0とするか）.
	*
	* @retval true		 読み込み成功.
	* @retval false		 読み込み失敗.
	*
	...
	*/
	float HeightMap::Height(const glm::vec3& pos) const {
		...
		// 「offset.x + offset.y < 1」ならば、offsetは左上の三角形領域に存在する.
	...
	*/
	bool HeightMap::CreateMesh(Mesh::Buffer& meshBuffer, const char* meshName, const char* texName) const {
		...
		m.texture[0] = Texture::Image2D::Create("Res/Images/Terrain_Ratio.tga");
		m.texture[1] = Texture::Image2D::Create("Res/Images/Terrain_Soil.tga");
		m.texture[2] = Texture::Image2D::Create("Res/Images/Terrain_Rock.tga");
	...
	*/
	bool HeightMap::CreateWaterMesh(Mesh::Buffer& meshBuffer, const char* meshName, float waterlevel) const
	{
		...
		cubeMapFiles.push_back("Res/Images/cubemap_px.tga");
		cubeMapFiles.push_back("Res/Images/cubemap_nx.tga");
		cubeMapFiles.push_back("Res/Images/cubemap_py.tga");
		cubeMapFiles.push_back("Res/Images/cubemap_ny.tga");
		cubeMapFiles.push_back("Res/Images/cubemap_pz.tga");
		cubeMapFiles.push_back("Res/Images/cubemap_nz.tga");
		m.texture[6] = Texture::Cube::Create(cubeMapFiles);
		m.texture[8] = Texture::Image2D::Create("Res/Images/Terrain_Water_Normal.tga");
	...
	*/
	glm::vec3 HeightMap::CalcNormal(int centerX, int centerZ) const {
		...
		const glm::ivec2 offsetList[] = { {0,-1},{1,-1},{1,0},{0,1},{-1,1},{-1,0},{0,-1} };
